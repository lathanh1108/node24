...
function initModels(sequelize) {
  ...
  restaurant.belongsToMany(user, { as: 'users_liked', through: like_res, foreignKey: "res_id", otherKey: "user_id" });
  restaurant.belongsToMany(user, { as: 'users_rate', through: rate_res, foreignKey: "res_id", otherKey: "user_id" });
  user.belongsToMany(food, { as: 'food_id_foods', through: order, foreignKey: "user_id", otherKey: "food_id" });
  user.belongsToMany(restaurant, { as: 'restaurant_like', through: like_res, foreignKey: "user_id", otherKey: "res_id" });
  user.belongsToMany(restaurant, { as: 'restaurant_rate', through: rate_res, foreignKey: "user_id", otherKey: "res_id" });
